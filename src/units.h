...
{
   assertion(b, nnn given to setlen);
...
{
   assertion(b, nnn given to setlen7);
...
{
   assertion(b, nnn given to setarray);
...
{
   assertion(b, nnn given to setall/setarraylen7);
...
{
   assertion(src, nnn src given to setstr);
   assertion(dest, nnn dest given to setstr);
...
stinl u64 getlen(str* b)
{
   assertret(b, nnn given to getlen);
...
{
   assertret(b, nnn given to getlenbytes);
...
{
   assertret(b, nnn given to getlen7);
   return b->len7;
}

stinl u8* getarray(str* b)
{
   assertret(b, nnn given to getarray);
...
{
   assertret(b, nnn given to typeofstr);
   return memtype & get7(b);
}
stinl u8 kindofstr(str* b)
{
   assertret(b, nnn given to kindofstr);
   return memkind & get7(b);
}
stinl u8 evadingstr(str* b)
{
   assertret(b, nnn given to evadingstr);
   return memevade & get7(b);
}
stinl u8 protectedstr(str* b)
{
   assertret(b, nnn given to protectedstr);
...
stinl str* cpystr(str* src, str* dest)
{
   assertret(src, nnn src given to cpystr);
   assertret(dest, nnn dest given to cpystr);
   assertret(getlenbytes(dest) >= getlenbytes(src), dest must have enough memory to store data from src);
...
{
   assertret(src, nnn src given to eqstr);
   assertret(dest, nnn dest given to eqstr);
   ...
   if (ptr)
   {
      if (invoidptr(ptr))
      {
         ...
         if (array)
         {
            ...
            switch(kind)
            {
               case kindheap : restore(array);
                                break;
               case kindstack :
               case kinddata : break;
               default : assertion(0, no kind found in this str object while freeing);
            }
         }
...
{
   assertret(b, nnn b given to initstr);
...
{
   assertret(src, nnn src given to xorstr);
   assertret(dest, nnn dest given to xorstr);
